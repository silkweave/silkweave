    ...
    <div className="flex flex-col h-screen max-w-3xl mx-auto px-4">
      <header className="py-6 border-b border-neutral-800">
        ...
        <p className="text-sm text-neutral-400 mt-1">
          <code className="text-xs bg-neutral-900 px-1.5 py-0.5 rounded">useChat</code> → custom transport → tRPC
          subscription → Silkweave streaming action →{' '}
          <code className="text-xs bg-neutral-900 px-1.5 py-0.5 rounded">streamText</code>
        </p>
      </header>
    ...
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm whitespace-pre-wrap leading-relaxed ${
          isUser ? 'bg-white text-black' : 'bg-neutral-900 text-neutral-100 border border-neutral-800'
        }`}
      >
        {children}
      </div>
