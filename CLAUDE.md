...
The MCP adapters can serve [Agent Skills](https://agentskills.io/specification) (`SKILL.md` directories) - versioned, digest-verified, installable via `npx silkweave skills sync`. Design tracks the MCP Skills-over-MCP working group's SEP-2640 resources baseline (full research + decisions: `plans/skills-over-mcp-exploration.md`); the extension methods (`skills/list`/`skills/get`) are deliberately NOT implemented while the draft churns.
