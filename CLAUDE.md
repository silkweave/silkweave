...
pnpm -F @silkweave/example-mcp http-multi-resource # One AS, N per-tenant protected resources on :8080
...
pnpm -F @silkweave/example-skills dev      # MCP HTTP server serving two Agent Skills on :8080 (try: npx silkweave skills list --url http://localhost:8080/mcp)

# Silkweave CLI (packages/silkweave - the `silkweave` npm package)
pnpm -F silkweave dev skills sync --url <mcp-url>   # install/update Agent Skills from a silkweave MCP server
pnpm -F silkweave dev proxy <mcp-url> [tool ...]    # universal cliProxy: any MCP server's tools as CLI commands
...
| `@silkweave/auth` | `packages/auth` | Auth - **resource-server core** (spec-required, `jose`-only): bearer-token validation (expiry + issuer binding RFC 9207, audience binding RFC 8707, step-up `scope` challenge SEP-2350) + protected resource metadata (RFC 9728, with `scopes_supported`). The OAuth 2.1 **authorization-server** proxy (PKCE, refresh tokens, CIMD, dynamic client registration) + persistence stores live behind the opt-in **`@silkweave/auth/oauth`** subpath, so a pure resource server never pulls the issuer machinery into its graph. The OAuth *types* (`OAuthRequest`/`OAuthResponse`/`OAuthProvider`) stay at the root because `AuthConfig.provider` references them. **(5.1)** `AuthConfig.resourceUrl` widened to `string | ResourceResolver` for multi-resource servers - see [Multi-resource OAuth](#multi-resource-oauth-one-as-n-protected-resources) below. |
| `@silkweave/mcp` | `packages/mcp` | MCP adapters - stdio, streamable HTTP, CLI proxy. **(4.0)** The Express `http()` server + its mountable handlers (`mcpTransport`, `oauthRoutes`, `protectedResourceMetadata`, `sideloadResource`, `mcpCors`, `authMiddleware`) live behind the **`@silkweave/mcp/server`** subpath - the package root is express-free (`stdio`, `registerTools`, result helpers, `filterActions`), so a stdio-only or serverless consumer never pulls express/cors. `express` + `cors` are **optional peer deps** (needed only for `/server`). The tool-registration + result helpers are also re-exported from the express-free **`@silkweave/mcp/tools`** subpath, which web-standard adapters (`@silkweave/edge`, `@silkweave/nextjs`) import. |
...
| `@silkweave/typegen` | `packages/typegen` | Type generator - emits `.d.ts` interfaces from action Zod schemas as plain strings (no `typescript` runtime/peer dependency - it never type-checked, so the compiler-API AST factory + printer were dropped in favor of direct string emission; `typescript` remains a devDep for this package's own build/typecheck only) |
...
| `cliProxy` | `@silkweave/mcp/cli-proxy` | `packages/mcp/src/adapter/cliProxy.ts` | MCP CLI proxy client (`commander` + `StreamableHTTPClientTransport`). Exposed from the dedicated `@silkweave/mcp/cli-proxy` subpath (not the package root) so importing the `stdio`/`http` servers never pulls the CLI client's `commander`, which is an **optional peer dep** of `@silkweave/mcp`. Auth passthrough via `headers` (record or thunk, resolved once per invocation before connecting), `requestInit`, `fetch`, `authProvider` (all forwarded to the transport); a failed connect prints a legible one-liner (auth-specific on 401/403) and root `--help`/`--version` degrade gracefully offline. Renders input fields listed in the tool's `_meta['silkweave/args']` (emitted by `registerTools()` from `Action.args`) as positional arguments in declared order - required (per JSON Schema `required`) as `<arg>`, optional as `[arg]`. |
...
MCP adapters (`stdio`, `http`) register actions as MCP tools using `PascalCase` names (via the shared `registerTools()`). **Two integration primitives** let Silkweave drop into an existing app: the **Node** handler `mcpTransport().post` (an Express `RequestHandler`, imported from **`@silkweave/mcp/server`** since 4.0; for express / fastify / NestJS) and the **Web-Standard** handler `edge().handler` (a `(Request) => Response`, for hono / Next.js / Cloudflare / Deno / Bun - express-free). The CLI adapter uses `kebab-case` for commands and maps Zod types to CLI options/arguments; `Action.args` (input-schema keys, in order) marks fields as positional arguments - honored by the local `cli()` adapter directly and carried across the wire to `cliProxy` via the MCP tool's `_meta['silkweave/args']` (emitted by `registerTools()`, spec-legal, ignored by non-silkweave clients). The `typegen` adapter uses `allActions: true` to bypass `isEnabled` filtering and generate types for all registered actions. The `trpc` adapter registers each action as a tRPC procedure at `camelCase(action.name)`, dispatching `action.kind` to `.query()` or `.mutation()`; the exported `InferTrpcRouter<typeof server>` type extracts a fully-typed `AppRouter` for `createTRPCClient<AppRouter>()`.
...
- `ToolCallEvent` / `OnToolCall` / `emitToolCall()` in `packages/core/src/util/telemetry.ts` - the per-invocation telemetry contract. `emitToolCall()` invokes a hook **fire-and-forget** (never awaited on the call path; sync throws and async rejections logged to stderr and swallowed). MCP adapters accept `onToolCall` (option on `stdio()`/`http()`/`mcpTransport()`/`edge()`) and emit from `registerTools()` **after result formatting**, so MCP events carry `resultBytes` (serialized raw-result size) and `sideloaded` (smart embedded-resource offload); `ok` is false on a thrown error (`errorCode` = `SilkweaveError.code`, else the error's name) or an `isError` tool result. Events carry `args` - the call's input: parsed (post-zod) on success/handler-error events, the **raw offered input** on validation-failure events; **unredacted** (like `context`, which carries the raw request incl. `Authorization`) - consumers must redact/truncate before persisting. **Input-validation failures emit too** (`ok: false`, stable `errorCode: 'INVALID_ARGUMENTS'`, `durationMs: 0`): the SDK rejects an invalid `tools/call` before the handler, so `emitInvalidArguments()` (`packages/mcp/src/handlers/prevalidate.ts`, exported from root + `/tools`) pre-validates **emit-only** in `mcpTransport()`/`edge()` - same schema, `safeParseAsync` on the verbatim `params.arguments`, mirroring the SDK so no call double-emits; the request always proceeds to the SDK's native rejection (wire behavior unchanged, seam survives an SDK major swap; `stdio` not covered). `@silkweave/nestjs` wires telemetry via DI: `forRoot({ telemetry: ClassToken })` (a `SilkweaveTelemetry` provider, resolved via `ModuleRef` lazily at first event) - MCP events come from the registrar seam, tRPC events from the synthesized action wrapper in `ControllerDiscovery` (gated to the `trpc` adapter context; guard denials count as failed calls; `args` = validated per-procedure input, already de-batched) - **exactly one event per call** on either transport. tRPC input-validation failures (which fire before the resolver, so the wrapper never runs) emit from the `trpc()` adapter's `onError` seam via `buildValidationErrorEmitter()` (`packages/nestjs/src/lib/validationTelemetry.ts`, internal), discriminated by `error.cause.name === 'ZodError'` (blind spot: a controller deliberately throwing a raw `ZodError` double-counts).
...
### Resources (binary results)

An action can return a **binary or text artifact** (screenshot PNG, PDF, JSON/markdown artifact, audio) instead of a JSON object. Declare the output with core's `binary()` schema helper and return a `resource()` (or a Web-Standard `File`/`Blob`, or bare `Uint8Array`/`ArrayBuffer` bytes - normalized via `toActionResource()`, with the `binary()` metadata as defaults for what the value doesn't carry):
...
Core primitives (`packages/core/src/util/resource.ts`): `resource()` / `ActionResource` (brand-symbol-detected value: `data: Uint8Array | string`, `mimeType`, `name?`, `description?`), `binary(meta)` (a Zod schema over resource-like values carrying `BinarySchemaMeta` defaults, detected via `isBinarySchema()`/`binarySchemaMeta()`), `toActionResource(value, defaults)` (async normalization, `undefined` for non-resource values - adapters call it on every result), `isTextMimeType()` (`text/*`, JSON, XML/SVG, JS, `+json`/`+xml` suffixes), `serializeResource()`/`deserializeResource()` (the `SerializedResource` JSON envelope: `{ kind: 'resource', mimeType, name?, description?, text? | base64? }`), and edge-safe `bytesToBase64`/`base64ToBytes`. Everything is Web-Standard (no Buffer/node imports) so it runs on edge/Workers.

Per-adapter delivery (**embed-only** - no hosted resource links):

| Adapter | Delivery |
|---------|----------|
| `stdio()`, `http()`, `edge()` (MCP) | `resourceToolResult()` maps mime-driven content blocks: `description` first as a `text` block, then raster `image/*` (png/jpeg/gif/webp) ⇒ `image` block (multimodal hosts show it to the model), `audio/*` ⇒ `audio` block, text-based mime ⇒ embedded `resource` with `text`, else embedded `resource` with base64 `blob`. A `toolResult` hook still wins; `_meta.disposition` cannot demote a resource result. `disposition: 'structured'` + `binary()` is rejected at registration (extended `validateActionDisposition()`), as is a `binary()` chunk schema. |
| `fastify()` (REST) | Raw bytes: `Content-Type` from the mime type, `Content-Disposition: inline; filename=` from `name`, `Content-Description` from `description`; OpenAPI response documents `type: string, format: binary` under the declared media type. |
| `cli()` | Piped stdout ⇒ raw bytes (`my-cli screenshot > shot.png`, status/description on stderr); interactive TTY ⇒ written to `--output <path>` (flag added for `binary()` actions) or a file named after the resource. Binary is never dumped onto a TTY. |
| `cliProxy` | Decodes `image`/`audio` blocks and non-text `blob` resources from the tool result; same pipe/`--output`/TTY behavior (`--output` added to every proxied command unless the schema claims the name). Text resources (incl. smart offloads) keep printing as text. |
| `trpc()`, `trpcFetch()` | JSON-only wire ⇒ the result ships as its `SerializedResource` envelope; `InferTrpcRouter` maps resource-like run return types to `SerializedResource` at the type level. |
| `typegen` | `binary()` outputs emit the `SerializedResource` interface literal (also in the generated `AppRouter`). |

Telemetry: a resource result's `resultBytes` is the payload byte length (not a JSON stringification) and `sideloaded` stays `false` (deliberate resources are not smart offloads). Streaming + resources don't mix in v1 (no binary chunk schemas).

> **NestJS:** `@Mcp({ resource })` / `@Trpc({ resource })` (`ResourceMetadata`: `mimeType?`/`name?`/`description?`) declare a controller route's result a resource; without the option, a reflected non-JSON `@Header('Content-Type', ...)` flips the route automatically (JSON Content-Types don't; never auto-applied to `async *` routes). The synthesized action gets `output: binary(meta)`; returned `StreamableFile`s are collected to bytes (`normalizeControllerResult()` in `packages/nestjs/src/lib/resource.ts` - explicit `type`/`disposition` options win as a named `resource()`, otherwise bare bytes let the declared defaults apply). Incompatible with `result: 'structured'` and with streaming routes (boot errors); `resource` wins over `output`.

### Skills (serving Agent Skills over MCP)

The MCP adapters can serve [Agent Skills](https://agentskills.io/specification) (`SKILL.md` directories) - versioned, digest-verified, installable via `npx silkweave skills sync`. Design tracks the MCP Skills-over-MCP working group's SEP-2640 resources baseline (full research + decisions: `plans/skills-over-mcp-exploration.md`); the draft extension methods are additionally available behind an **experimental opt-in flag** (see the SEP-2640 bullet below).

```typescript
import { defineSkill, silkweave } from '@silkweave/core'
import { http } from '@silkweave/mcp/server'

silkweave({ name: 'team-skills', version: '1.0.0', description: '...' })
  .adapter(http({ port: 8080, host: 'localhost', skills: [defineSkill({ dir: './skills/commit-message' })] }))
  .start()
```

- **Core seam** (`packages/core/src/util/skill.ts`): `Skill`/`SkillFile`/`SkillDefinition` types + `defineSkill()` (sync capture; exactly one of `dir` or inline `files` - the latter for filesystem-less runtimes) + `isResolvedSkill()`. Zero runtime beyond validation, kept in core so adapters need no type dependency on `@silkweave/skills`.
- **Resolution** (`packages/skills/src/util/resolve.ts`): `resolveSkills()` loads `dir` (lazy `node:fs` import - the package root stays edge-importable), parses frontmatter (`yaml`), validates the spec (name shape/length, dir-name match with `defineSkill({ name })` as the escape hatch, description 1-1024), computes per-file + aggregate `sha256:<hex>` digests (Web Crypto), rejects traversal paths (`assertSafeSkillPath`) and duplicate names. Version = `defineSkill({ version })` ?? frontmatter `metadata.version` (the spec has no first-class version field).
- **Serving** - a `skills` option on `stdio()`/`http()`/`mcpTransport()`/`edge()`, prepared once at start by `prepareSkills()` (`packages/mcp/src/handlers/skills.ts`, exported from root + `/tools`; lazy-imports `@silkweave/skills`, an **optional peer** - a legible `missing_dependency` error otherwise). Three surfaces per request: (1) **`skill://<name>/<path>` resources** via `registerSkillResources()` (`@silkweave/skills/mcp`) - the SEP-2640 baseline, plain `resources/list`/`read`; (2) **`ListSkills`/`GetSkill` tools** (`skillActions()`) - ordinary actions (tagged `silkweave/skills`) returning the digest manifest / full payload (text as `text`, binary as `base64`), so filters/auth/telemetry apply and any current client can install; (3) **server `instructions`** (`skillInstructions()`) announcing the skills (the WG-validated activation pattern - previously no adapter passed `instructions` at all). **Filter gating:** when `filterActions` drops the skill actions for a request, `SkillServing.visible()` also hides the resources + instructions - a filter that hides the tools never leaks the files via `resources/read`. `mcpTransport()` now returns `{ post, ready }`; `http()` awaits `ready` so a bad skill fails at boot, per-request consumers (nestjs `mcp()`) surface it as a 500.
- **CLI client** (`packages/silkweave/src/lib/skills/`): `sync` diffs the remote manifest against the target's `.silkweave-lock.json` (`diffSkills()`: `missing`/`outdated`/`up-to-date`/`held`/`orphaned`), installs under `<target>/<name>/` with every file re-verified against its digest and name/path revalidated client-side (a hostile server cannot escape the target), removes files a previous version tracked that the new one dropped, `--prune` removes orphans. `pin`/`unpin` hold a skill (`held` diffs are skipped; explicit `install` of a held skill errors). One server per target/lockfile (mismatch warns). Default target `~/.claude/skills`.
- **Wire contract** (`packages/skills/src/util/manifest.ts`): `SkillManifestEntry` (metadata + per-file digests - mirrors SEP-2640's listing shape) from `ListSkills`; `SkillPayload` (files with `text`/`base64`) from `GetSkill`; the CLI checks `payload.digest === manifest.digest` to catch a server changing mid-sync.
- **SEP-2640 extension (EXPERIMENTAL, opt-in)** - `skillsExtension: true` on `stdio()`/`http()`/`mcpTransport()`/`edge()` additionally serves the draft extension: `capabilities.extensions['io.modelcontextprotocol/skills']` (`{ directoryRead: false }` - `resources/directory/read` not implemented) + `skills/list` (entries `{ uri: skill://<name>/SKILL.md, frontmatter, resources: [{uri, digest}] }`) + `skills/get` by URI (`registerSkillExtension()` in `@silkweave/skills/mcp`; entry shape + `aggregateDigest()` in `packages/skills/src/util/extension.ts`). Gated by the same `visible()` filter rule as the rest of the surface. Off by default because the draft churns (the `skill://index.json` design was replaced by these methods in July 2026) - shapes may change with the draft. The **CLI is extension-aware**: `skillSource()` (`packages/silkweave/src/lib/skills/client.ts`) prefers `skills/list` + `resources/read` when the server declares the capability - so `silkweave skills sync` installs from ANY SEP-2640 server (e.g. Hugging Face MCP), not just silkweave ones; foreign listings get an aggregate digest computed client-side with the same canonical formula (`aggregateDigest` sorts SKILL.md-first internally), so lockfile identities are stable across both consumption paths. Tool path remains the fallback.
- **Claude Code marketplace bridge (Phase 3)** - public skills get the native `/plugin` experience (real versioning, `/plugin update`) via npm, no repo exposure, no registry of ours. **Pack:** `silkweave skills pack <dir...>` wraps skill dir(s) into a skills-only plugin package ready for `npm publish` - `package.json` + `.claude-plugin/plugin.json` + `skills/<name>/...` (`pluginFiles()` in `packages/skills/src/util/marketplace.ts` is the pure layout; `packSkill()` in `packages/silkweave/src/lib/skills/pack.ts` does fs + the registry check). Multiple dirs pack into ONE **multi-skill plugin** (requires `--package`; plugin name = package basename via `pluginNameForPackage()`; all skills must agree on a version). Version comes from frontmatter `metadata.version` (required); package name = `--package` ?? `metadata.npmPackage` ?? `<name>-skill`; packing **refuses an already-published version** (registry lookup; `--force` overrides, `--registry` for private registries, unreachable registry warns and proceeds); the out dir (default `dist/<name>-plugin`) is only overwritten when it is a previous pack output. **Serve:** the `skillsMarketplace` option on `http()`/`edge()`/`mcpTransport()` (`{ name?, owner, description?, registry? }`, name defaults to the server name) answers `GET /.claude-plugin/marketplace.json` (`MARKETPLACE_PATH`) with npm-sourced plugin entries for every served skill carrying an `npmPackage` (`defineSkill({ npmPackage })` ?? frontmatter `metadata.npmPackage`, threaded onto `Skill` by `resolveSkill`; document built by `marketplaceJson()`, pre-serialized onto `SkillServing.marketplace`; skills sharing an `npmPackage` collapse into one multi-skill plugin entry named `pluginNameForPackage()`). The published demo: `@silkweave/example-plugin` bundles both `examples/skills` skills. Served **unauthenticated** (like `/.well-known/` - it only points at already-public npm packages; npm sources are the only kind emitted since a URL-added marketplace can't resolve relative paths). Boot-errors when `skills` is unset or no served skill is npm-published. Consumers: `/plugin marketplace add https://<host>/.claude-plugin/marketplace.json`.
- Digests bind approval to content (supporting-file rotation is caught); they are **not signatures** - skills are code, install only from trusted servers.

### Multi-resource OAuth (one AS, N protected resources)

A server fronting N tenants needs a distinct URL per tenant (Claude clients dedupe MCP connectors **by URL**), and each URL must be its own OAuth **protected resource** with its own token audience - so a token minted for tenant A is rejected when replayed at tenant B. There is still exactly one authorization server.

- **Resource-server side** - `AuthConfig.resourceUrl` accepts a `ResourceResolver` (`(ResourceRequest, SilkweaveContext) => string | undefined`) in addition to a `string`. `packages/auth/src/resolve.ts` holds the primitives: `toResourceRequest()` (normalizes the adapter-forked `request` - Fetch `Request` / Express / Fastify / bare `IncomingMessage` - into `{ url, headers }`), `resolveResourceUrl()`, `normalizeResourceUri()` (RFC 8707 §2 canonical form: lowercased scheme/host, default port stripped, no fragment, bare-origin trailing slash stripped - the classic bug, since SDKs ship `new URL(resource)` stringifications), `protectedResourceMetadataUrl()` (RFC 9728 **path-insertion** form), and `pathResolver({ origin, match })` - the safe combinator that builds identifiers from a **configured** origin, never the request's, so a spoofed `Host` cannot steer the advertised metadata URL or the expected audience. The resolver is deliberately **synchronous** (shape/identity mapping); async policy belongs in the AS's `allowedResources`. `validateToken` resolves once per call, returns the resolved `resource` on `ValidateResult`, and derives `expectedAudience` from it. **Challenge-URL asymmetry is deliberate**: `string` configs keep the historical **append** form (load-bearing for the nestjs basePath mount, and identical to insertion for a path-less resource); resolver-resolved resources get RFC 9728's **insertion** form, which is what the SDK probes. A resolver returning `undefined` behaves exactly as an unset `resourceUrl` (challenge without `resource_metadata`, no *default* audience check; an explicit `audience` still applies) - fail-open, deliberately, because adapters guard non-resource routes too (sideload `/resource/:id`) and failing closed would 401 every sideload fetch.
- **Per-request metadata** - `resolveProtectedResourceMetadata(auth, request, context)` (`packages/auth/src/metadata.ts`) strips the well-known prefix and replays the remainder through the resolver as the as-if resource request, so a resolver never learns the prefix exists; it compares **pathnames** (proxy-robust, no `trust proxy` dependency) and returns `undefined` -> 404 for an unknown sub-resource or a resolver whose answer addresses a different path.
- **Authorization server** - `OAuthProxyConfig.allowedResources?: string[] | ((resource: string) => boolean | Promise<boolean>)` bounds the reachable `aud` space; the RFC 8707 indicator is threaded authorize -> `PendingAuthData.resource` -> `AuthCodeData.resource` -> mint, and persisted on `RefreshTokenData.resource` so rotation preserves the audience (a mismatched `resource` on refresh is `invalid_target` - a refresh token must not be a lateral move). The allow-list is re-checked **at redemption**, not only at `/authorize`, so omitting the indicator on the authorize leg cannot smuggle one in at `/token`. Exactly **one** resource per grant (repeats rejected); `body.resource` that is not a single string is rejected rather than stringified (Express `urlencoded` parses repeats to arrays behind the declared `Record<string, string>`). `iss` stays `config.resourceUrl` - the AS identity does not fragment.
- **`verifyToken`** - without `allowedResources` the historical double pin (`issuer` + `audience`) is kept **verbatim**. With it, the audience pin is replaced by an issuer-only pin plus a manual check that is deliberately *stricter*: `jose`'s `audience` option **requires the claim to be present**, so dropping it without a shape check would accept an `aud`-less token, which `validateToken` treats as a pass. The check therefore requires a **single non-empty string** `aud` (rejecting missing, empty, and array forms - the proxy never mints arrays) and then membership. The membership check is not defence against key compromise (an attacker with the key mints allowed audiences trivially); its value is **revocation** and misconfig containment. Returning `aud`/`iss` in the `AuthInfo` is the load-bearing part - it turns `validateToken`'s per-request audience binding from vacuous (today, for proxy tokens, the binding lives only inside `jwtVerify`) into the actual confused-deputy defence.
- **`aud` is not membership** - the AS mints `aud: …/tenantA` for any authenticated upstream user who asks, including one with no rights in A. `resource` is where a token may be *presented*, not what its subject may *do* there; per-tenant authorization stays the resource server's job on every call. `ValidateResult.resource` exists so that check does not have to re-parse the request URL.
- **Adapters** - the well-known mount becomes a wildcard when `resourceUrl` is a resolver: Express 5 optional-splat (`'/.well-known/oauth-protected-resource{/*resource}'`) in `http()` and nestjs, a prefix match on `validPaths` in `edge()`, and a `'/*'` route in `fastify()`. **nestjs additionally root-mounts the RFC 8414 insertion-form AS metadata** (`/.well-known/oauth-authorization-server${basePath}`) alongside its basePath append-form route: the SDK probes insertion form for a path'd issuer and never the append form, and on discovery failure falls back to root-absolute `/authorize` + `/register`, so without this AS discovery simply fails.
- Tests: `packages/auth/src/resolve.test.ts`, `metadata.test.ts`, the resolver suite in `validate.test.ts`, and `provider/proxy.test.ts` (**new** - the proxy previously had no tests; drives the full authorize -> callback -> token -> refresh -> verify flow against a stubbed upstream).
...
- `rebind.ts` (`packages/nestjs/src/lib/rebind.ts`) - `invokeRebound()` reconstructs the method's positional args from the flat tool input per the discovery-time `Binding[]` plan (scalar field, whole-DTO object, path-params object, or `@Req`/`@Headers`/`@Ip` sourced from the request stand-in, plus `@Res` sourced from the forked `response`), runs parameter-bound pipes (unless `pipes: 'skip'`), then `method.apply(instance, args)`. `@Res()` resolves to the **real Express response over tRPC** (the `trpc()` adapter forks `response: res` into the context, so `@Res({ passthrough: true })` can set session cookies/headers) and to `undefined` over MCP (no HTTP response). Globally-registered `ValidationPipe`/interceptors/exception filters do **not** run - only guards and param-bound pipes. **Security caveat:** a controller that relies on a global `ValidationPipe({ whitelist: true })` to strip fields (e.g. `role`/`isAdmin`) before a guard or handler reads them will, over MCP, see the raw tool input with those fields intact (`populateRequestSlots` copies them straight from the input). Enforce such invariants with a param-bound pipe or an explicit check in the action, not a global pipe. A pipe that fails to construct (DI-dependent) is skipped with a `Silkweave` `logger.warn` rather than silently.
...
Each publishable package (`packages/*`, except the `silkweave` CLI - bin-only, no library exports) resolves to **build output for everyone except our own tooling**, while in-repo tooling resolves to **TS source** - so cross-package edits need no rebuild, but external installs/`pnpm link`/registry consumers always get `build/`:
...
- **How it works**: `scripts/indexnow-submit.ts` (run via `pnpm indexnow`) POSTs the URL list to `api.indexnow.org` with the site key. IndexNow verifies ownership by fetching `https://www.silkweave.dev/<key>.txt`.
- **Key storage**: the key file is committed at `website/static/<key>.txt` (the website's public dir is `static/`, not Astro's default `public/`), served at the default IndexNow key location. IndexNow keys are public by protocol - the key file must be world-readable on the host anyway - so committing it is fine. The submit script auto-discovers the key from that file (a `<key>.txt` whose content equals its basename), or takes `$INDEXNOW_KEY` as an override. The website package is `private: true`, so nothing here can reach npm.
- Submissions are idempotent; re-submitting an unchanged URL is harmless but pointless.
