...
- **Claude Code marketplace bridge (Phase 3)** - public skills get the native `/plugin` experience (real versioning, `/plugin update`) via npm, no repo exposure, no registry of ours. **Pack:** `silkweave skills pack <dir>` wraps one skill dir into a skills-only plugin package ready for `npm publish` - `package.json` + `.claude-plugin/plugin.json` + `skills/<name>/...` (`pluginFiles()` in `packages/skills/src/util/marketplace.ts` is the pure layout; `packSkill()` in `packages/silkweave/src/lib/skills/pack.ts` does fs + the registry check). Version comes from frontmatter `metadata.version` (required); package name = `--package` ?? `metadata.npmPackage` ?? `<name>-skill`; packing **refuses an already-published version** (registry lookup; `--force` overrides, `--registry` for private registries, unreachable registry warns and proceeds); the out dir (default `dist/<name>-plugin`) is only overwritten when it is a previous pack output. **Serve:** the `skillsMarketplace` option on `http()`/`edge()`/`mcpTransport()` (`{ name?, owner, description?, registry? }`, name defaults to the server name) answers `GET /.claude-plugin/marketplace.json` (`MARKETPLACE_PATH`) with npm-sourced plugin entries for every served skill carrying an `npmPackage` (`defineSkill({ npmPackage })` ?? frontmatter `metadata.npmPackage`, threaded onto `Skill` by `resolveSkill`; document built by `marketplaceJson()`, pre-serialized onto `SkillServing.marketplace`). Served **unauthenticated** (like `/.well-known/` - it only points at already-public npm packages; npm sources are the only kind emitted since a URL-added marketplace can't resolve relative paths). Boot-errors when `skills` is unset or no served skill is npm-published. Consumers: `/plugin marketplace add https://<host>/.claude-plugin/marketplace.json`.
