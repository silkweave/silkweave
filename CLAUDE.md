...
| `cliProxy` | `@silkweave/mcp/cli-proxy` | `packages/mcp/src/adapter/cliProxy.ts` | MCP CLI proxy client (`commander` + `StreamableHTTPClientTransport`). Exposed from the dedicated `@silkweave/mcp/cli-proxy` subpath (not the package root) so importing the `stdio`/`http` servers never pulls the CLI client's `commander` + `@clack/prompts`; both are **optional peer deps** of `@silkweave/mcp`. |
...
- `createLogger()` / `buildLogLevels()` in `packages/logger/src/logger.ts` (the `@silkweave/logger` package) - `createLogger()` is a **zero-dependency** structured logger that writes JSON lines to a stream (default `process.stdout`, `false` to discard), honoring a `level` threshold and `onLog`/`onProgress` callbacks; it is the default logger forked into the action context by the `stdio`/`http`/`vercel` MCP adapters (`stream: false` or `process.stderr`, with `onLog` forwarding to MCP `notifications/message`). `buildLogLevels()` builds a log-level record from a single callback. **No pino** - the package's only hard dep is `zod`.
- `createCLILogger()` in `packages/logger/src/cli.ts` - terminal-UI logger via `@clack/prompts` (an **optional peer** of `@silkweave/logger`), used by `@silkweave/cli` and `@silkweave/mcp`'s cliProxy. `@silkweave/core` does **not** depend on `@silkweave/logger` - the logger reaches actions purely through `context.get('logger')`, so a core-only consumer (e.g. the `@Mcp()` decorator import path in `@silkweave/nestjs`) pulls neither pino nor clack.
