...
export interface Silkweave<Actions extends Record<string, Action> = {}> {
  ...
  action: <A extends Action<any, any, string, any>>(action: A) => Silkweave<Actions & Record<A['name'], A>>
  actions: <Arr extends readonly Action[]>(actions: Arr) => Silkweave<Actions & { [K in Arr[number] as K['name']]: K }>
