...
describe('edge skills serving', () => {
  it('appends ListSkills/GetSkill to the tool list', async () => {
    ...
    const body = (await (await app.handler(post(rpc('tools/list')))).json()) as {
  ...
  it('serves skill files as skill:// resources', async () => {
    const app = await startEdge()
    const listing = (await (await app.handler(post(rpc('resources/list')))).json()) as {
    ...
    const read = (await (
      await app.handler(post(rpc('resources/read', { uri: 'skill://deploy-checklist/SKILL.md' })))
    ).json()) as {
  ...
  it('returns the digest-carrying manifest and payload through the tools', async () => {
    const app = await startEdge()
    const listRes = (await (
      await app.handler(post(rpc('tools/call', { name: 'ListSkills', arguments: {} })))
    ).json()) as {
      result: { content: { text: string }[] }
    }
    const manifest = JSON.parse(listRes.result.content[0].text) as {
    ...
    const getRes = (await (
      await app.handler(post(rpc('tools/call', { name: 'GetSkill', arguments: { name: 'deploy-checklist' } })))
    ).json()) as {
      result: { content: { text: string }[] }
    }
    const payload = JSON.parse(getRes.result.content[0].text) as {
  ...
  it('serves the SEP-2640 extension when skillsExtension is enabled', async () => {
    ...
    const got = (await (
      await app.handler(post(rpc('skills/get', { uri: 'skill://deploy-checklist/SKILL.md' })))
    ).json()) as {
      result: { skill: { uri: string } }
    }
    expect(got.result.skill.uri).toBe('skill://deploy-checklist/SKILL.md')

    const missing = (await (await app.handler(post(rpc('skills/get', { uri: 'skill://nope/SKILL.md' })))).json()) as {
  ...
  it('serves the plugin marketplace for npm-published skills', async () => {
    ...
    const doc = (await response.json()) as {
      name: string
      owner: { name: string }
      plugins: { name: string; source: object; version?: string }[]
    }
    ...
    const rejected = await app.handler(
      new Request('http://localhost/.claude-plugin/marketplace.json', { method: 'POST' })
    )
  ...
  it('hides resources and instructions when a filter drops the skill tools', async () => {
    const app = await startEdge({
      filterActions: (all, request) =>
        request.headers['x-role'] === 'insider'
          ? all
          : all.filter((action) => !action.tags?.includes('silkweave/skills'))
    })
    const outsiderTools = (await (await app.handler(post(rpc('tools/list')))).json()) as {
      result: { tools: { name: string }[] }
    }
    ...
    const insider = (await (await app.handler(post(rpc('resources/list'), { 'x-role': 'insider' }))).json()) as {
      result: { resources: { uri: string }[] }
    }
