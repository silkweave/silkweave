...
describe('rpcInfo', () => {
  it('extracts the JSON-RPC method', () => {
    ...
    expect(rpcInfo({ jsonrpc: '2.0', method: 'notifications/initialized' })).toEqual({
      method: 'notifications/initialized'
    })
  })

  it('extracts toolName for tools/call only', () => {
    expect(rpcInfo({ method: 'tools/call', params: { name: 'UsersGet', arguments: {} } })).toEqual({
      method: 'tools/call',
      toolName: 'UsersGet'
    })
...
describe('filterErrorResponse', () => {
  it('maps a SilkweaveError to its statusCode with a JSON-RPC error body', () => {
    const { status, body } = filterErrorResponse(new SilkweaveError('invalid api key', 'invalid_key', 401), {
      id: 7,
      method: 'tools/list'
    })
